...
#ifdef __cpp_char8_t
  void Append(const std::u8string_view value) {
    Append(std::string_view(reinterpret_cast<const char*>(value.data()), value.size()));
  }
  void Append(const char8_t* value) {
    Append(reinterpret_cast<const char*>(value));
  }
  void Append(const std::u8string& value) {
    Append(std::u8string_view(value));
  }
#endif
